...
file_source.onchange = function handleUpload() {
	...
	if(file_source.length == 0) {
		...
		copyFile(original_path, `./files/${new_hashed_filename}`)
		.then(() => {
			console.log("Done copying file");
			let git_cmds = `cd ${path.join(__dirname, "../files")} && git add ./${new_hashed_filename} && git commit -m "${new_hashed_filename}" && git push origin HEAD`;
